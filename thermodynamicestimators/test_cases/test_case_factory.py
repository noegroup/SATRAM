...
def make_double_well_1d_dataset():
    ...
    sampled_positions = get_data(sampler, potential_fn, biases)

    return test_case.TestCase(potential_fn, biases, sampled_positions, sampling_range)
...
def make_double_well_2d_dataset():
    ...
    sampled_positions = get_data(sampler, potential_fn, biases, initial_coordinates)
...
def get_data(sampler, potential_fn, biases, initial_coordinates):
    ...
    for k, bias in enumerate(biases):
        ...
        sampled_positions.append(samples)

    return torch.stack(sampled_positions)
